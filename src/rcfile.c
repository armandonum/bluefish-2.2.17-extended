...
static void
free_configlist(GHashTable * hasht)
{
...
GHashTable *
make_config_list_item(GHashTable * config_hasht, void *pointer_to_var, unsigned char type_of_var,
					  gchar * name_of_var, gint len)
{
	...
	/*g_print("make_config_list_item, call g_hash_table_replace() for hashtable %p with name %s and config_list_item=%p var pointer=%p\n",config_hasht,name_of_var,config_list_item, pointer_to_var);*/
	g_hash_table_replace(config_hasht, name_of_var, config_list_item);
	/*g_print("return hashtable %p\n",config_hasht);*/
	return config_hasht;
}
...
props_init_main(GHashTable * config_rc)
{
	...
	init_prop_string(&config_rc, &main_v->props.editor_font_string, "editor_font_string:", "Monospace Regular 13");
#else
	...
	init_prop_arraylist(&config_rc, &main_v->props.external_bookmarkcommands, "external_bookmarkcommands:", 0, TRUE);
...
/* the array has 4 elements:
 - the regex
 - the regex group number that has the filename
 - the regex group number that has the line number
 - the regex group number that has the output
...
GList * update_outputbox(GList *current, gboolean overwrite) {
	...
	defaults =
		g_list_append(defaults,
						array_from_arglist(_("pylint"), "([^:]+):([0-9]+):[0-9]+:(.*)", "1",
										 "2", "3", "pylint '%f'|", NULL));
	defaults =
		g_list_append(defaults,
						array_from_arglist(_("ctags"), "([^\\t ]+[ \\t]+[^\\t ]+[ \\t]+)([0-9]+)[ \\t]+([^ \\t]+)(.*)", "3",
										 "2", "4", "ctags -xu '%f'|", NULL));
#endif
...
GList *update_bookmarkcommands(GList *current, gboolean overwrite) {
	GList *defaults = NULL, *retlist;
	defaults = g_list_append(defaults,
						array_from_arglist(_("Generate from ctags"), "([^\\t ]+[ \\t]+[^\\t ]+[ \\t]+)([0-9]+)[ \\t]+([^ \\t]+)(.*)", "3",
										 "2", "4", "ctags -xu --kinds-c=ft --kinds-Python=cfm --kinds-PHP=cf '%f'|", "#444477", NULL));
	retlist = update_externals(current, defaults, overwrite, 8, 7);
	free_arraylist(defaults);
	return retlist;
}
...
GList *update_filters(GList *current, gboolean overwrite)
{
	...
	defaults =
		g_list_append(defaults,
					  array_from_arglist(_("js-beautify "), "|js-beautify -t -n|", NULL));
#ifdef WIN32
	defaults =
		g_list_append(defaults,
...
rcfile_parse_main(void)
{
	...
	if (!main_v->props.external_command)
		main_v->props.external_command = update_commands(main_v->props.external_command, TRUE);
	if (!main_v->props.external_filter)
		main_v->props.external_filter = update_filters(main_v->props.external_filter, TRUE);
	if (!main_v->props.external_outputbox)
		main_v->props.external_outputbox = update_outputbox(main_v->props.external_outputbox, FALSE);
	if (!main_v->props.external_bookmarkcommands)
		main_v->props.external_bookmarkcommands = update_bookmarkcommands(main_v->props.external_bookmarkcommands, FALSE);	
...
return_globalsession_configlist(gboolean init_values)
{
	...
	init_prop_integer(&config_rc, &main_v->globses.bookmarks_follow_document, "bookmarks_follow_document:", 1,
					  init_values);
	init_prop_integer(&config_rc, &main_v->globses.bookmarks_follow_cursor, "bookmarks_follow_cursor:", 1,
					  init_values);
...
return_session_configlist(GHashTable * configlist, Tsessionvars * session)
{
	...
	init_prop_integer(&configlist, &session->snr3_bookmarkpermanent, "snr_bookmarkpermanent:", 0, FALSE);
...
void props_free()
{
	...
	free_arraylist(external_bookmarkcommands);
