	...
	while (c != '\0') {
		...
		if (count > requested_size && startws!=WS_POS_UNDEFINED) {
			...
			if (starti == endi || endi==-1) {
				...
				tmp1 = buf+utf8_charoffset_to_byteoffset_cached(buf, starti-start);
				tmp2 = buf+utf8_charoffset_to_byteoffset_cached(buf, endi-start);
...
convert_identing(Tdocument * doc, gboolean to_tabs)
{
	...
	utf8_offset_cache_reset();
	tabsize = doc_get_tabsize(doc);
	DEBUG_MSG("convert_identing, got tabsize %d\n",tabsize);
	doc_unre_new_group(doc);
	while (buf[i] != '\0') {
		switch (buf[i]) {
		case '\n':
	...
	g_free(buf);
	doc_unre_new_group(doc);
	DEBUG_MSG("convert_identing, done\n");
}
