...
typedef struct {
	...
	GtkWidget *menu;
	GSList *group;
	GSList *spelllangs; /* a list of strings */
} Tdictlist;
...
{
	Tdictlist *dl = data;
	dl->spelllangs = g_slist_prepend(dl->spelllangs, g_strdup((gpointer)lang_tag));
}

static void 
add_dicts_to_menu(Tdictlist *dl) {
	GSList *tmpslist = dl->spelllangs;
	for (tmpslist = dl->spelllangs; tmpslist; tmpslist = tmpslist->next) {
		const gchar *lang_tag = (const gchar *)tmpslist->data;
		GtkWidget *menuitem;
...
static gint
spelllang_sort_func(gconstpointer a,gconstpointer b) {
	return -1 * g_strcmp0((const gchar *)a, (const gchar *)b);
}
...
bftextview2_populate_suggestions_popup(GtkMenu * menu, Tdocument * doc)
{
	...
	dl.spelllangs = g_slist_sort(dl.spelllangs, spelllang_sort_func);
	add_dicts_to_menu(&dl);
	g_slist_free_full(dl.spelllangs, g_free);
