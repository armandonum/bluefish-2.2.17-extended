...
typedef void (* set_listpref_strarr_in_list_func)(GtkTreeIter * iter, gchar ** strarr, Tprefdialog * pd);
...
static void
pref_apply_change(GtkListStore * lstore, gint arr_pointerindex, Tprefapplytype type, gchar * path, gchar * newval,
				  gint index)
{
	gchar **strarr;
	GtkTreeIter iter;
	...
	if (tpath && gtk_tree_model_get_iter(GTK_TREE_MODEL(lstore), &iter, tpath)) {
		gchar *oldval;
		gtk_tree_model_get(GTK_TREE_MODEL(lstore), &iter, arr_pointerindex, &strarr, -1);
		g_print("pref_apply_change, lstore=%p, index=%d, type=%d, got strarr=%p\n", lstore, index, type,
				  strarr);
		if (!strarr) {
			g_warning("pref_apply_change, got NULL strarr, probably a bug in the preferences code!\n");
			return;
		}
...
/********************************* 
 * the following functions are used by all of the preferences that have a GList 
 * of gchar** items that are edited in a listview widet
 * we name them 'listpref' and they all use a Tlistpref struct to hold the
 * essential information so we can use general functions
 **********************************/

static void
listpref_delete_strarr(Tprefdialog * pd, Tlistpref * lp, gint pointercolumn)
{
	GtkTreeIter iter;
...
static void
listpref_add_new(Tprefdialog * pd, Tlistpref *lp, guint strarr_num_items, set_listpref_strarr_in_list_func set_listpref_strarr_in_list)
{
	gchar **strarr;
	GtkTreeIter iter;
	strarr = pref_create_empty_strarr(strarr_num_items);
	gtk_list_store_append(GTK_LIST_STORE(lp->lstore), &iter);
	set_listpref_strarr_in_list(&iter, strarr, pd);
	*lp->thelist = g_list_append(*lp->thelist, strarr);
	lp->insertloc = -1;
}


static void
listpref_duplicate(Tprefdialog * pd, Tlistpref * lp, gint pointercolumn, set_listpref_strarr_in_list_func set_listpref_strarr_in_list)
{
	GtkTreeIter iter;
	GtkTreeSelection *select;
	lp->insertloc = -1;
	select = gtk_tree_view_get_selection(GTK_TREE_VIEW(lp->lview));
	if (gtk_tree_selection_get_selected(select, NULL, &iter)) {
		gchar **strarr, **newstrarr;
		gtk_tree_model_get(GTK_TREE_MODEL(lp->lstore), &iter, pointercolumn, &strarr, -1);
		newstrarr = g_strdupv(strarr);
		
		/* re-use iter for the insert location */
		gtk_list_store_append(GTK_LIST_STORE(lp->lstore), &iter);
		set_listpref_strarr_in_list(&iter, newstrarr, pd);
		*lp->thelist = g_list_append(*lp->thelist, newstrarr);
	}
}

static void
listpref_reload(Tprefdialog * pd, Tlistpref * lp, GList *list, guint strarr_num_items, set_listpref_strarr_in_list_func set_listpref_strarr_in_list)
{
	GList *tmplist = g_list_first(list);
	lp->insertloc = -1;
	gtk_list_store_clear(GTK_LIST_STORE(lp->lstore));
	while (tmplist) {
		gint arrcount;
		gchar **strarr = (gchar **) tmplist->data;
		arrcount = g_strv_length(strarr);
		if (arrcount == strarr_num_items) {
			GtkTreeIter iter;
			gtk_list_store_append(GTK_LIST_STORE(lp->lstore), &iter);
			set_listpref_strarr_in_list(&iter, strarr, pd);
		}
...
{
	listpref_add_new(pd, &pd->fad, 3, set_fileassociation_strarr_in_list);
}
...
delete_fileassociation_lcb(GtkWidget *wid, Tprefdialog *pd)
{
	...
	listpref_delete_strarr(pd, &pd->fad, 3);
...
{
	listpref_reload(pd, &pd->bd, list, 4, set_extcommands_strarr_in_list);
}

static void
add_new_extcommands_lcb(GtkWidget * wid, Tprefdialog * pd)
{
	listpref_add_new(pd, &pd->bd, 4, set_extcommands_strarr_in_list);	
}

static void
delete_extcommands_lcb(GtkWidget * wid, Tprefdialog * pd)
{
	listpref_delete_strarr(pd, &pd->bd, 4);
}

static void 
duplicate_extcommands_lcb(GtkWidget *wid, Tprefdialog *pd)
{
	listpref_duplicate(pd, &pd->bd, 4, set_extcommands_strarr_in_list);
}
...
create_extcommands_gui(Tprefdialog * pd, GtkWidget * vbox1)
{
	...
	hbox = gtk_hbox_new(FALSE, 0);
	gtk_box_pack_start(GTK_BOX(vbox1), hbox, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Add entry"), GTK_STOCK_ADD, G_CALLBACK(add_new_extcommands_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Delete entry"), GTK_STOCK_DELETE, G_CALLBACK(delete_extcommands_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Duplicate entry"), NULL, G_CALLBACK(duplicate_extcommands_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Update defaults"), GTK_STOCK_REFRESH, G_CALLBACK(updatedefaults_extcommands_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Reset all"), GTK_STOCK_CONVERT, G_CALLBACK(reset_extcommands_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
}
...
{
	listpref_add_new(pd, &pd->ed, 3, set_external_filters_strarr_in_list);
}

static void
delete_external_filter_lcb(GtkWidget * wid, Tprefdialog * pd)
{
	listpref_delete_strarr(pd, &pd->ed, 3);
}

static void 
duplicate_external_filter_lcb(GtkWidget *wid, Tprefdialog *pd)
{
	listpref_duplicate(pd, &pd->ed, 3, set_external_filters_strarr_in_list);
}

static void
reload_extfilters(Tprefdialog * pd, GList *list)
{
	listpref_reload(pd, &pd->ed, list, 3, set_external_filters_strarr_in_list);
}
...
/*	label = gtk_label_new(NULL);
	...
	hbox = gtk_hbox_new(FALSE, 0);
	gtk_box_pack_start(GTK_BOX(vbox1), hbox, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Add entry"), GTK_STOCK_ADD, G_CALLBACK(add_new_external_filter_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Delete entry"), GTK_STOCK_DELETE, G_CALLBACK(delete_external_filter_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Duplicate entry"), NULL, G_CALLBACK(duplicate_external_filter_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Update defaults"), GTK_STOCK_REFRESH, G_CALLBACK(updatedefaults_extfilters_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Reset all"), GTK_STOCK_CONVERT, G_CALLBACK(reset_extfilters_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
}
...
{
	listpref_add_new(pd, &pd->od, 7, set_outputbox_strarr_in_list);
}

static void
delete_outputbox_lcb(GtkWidget * wid, Tprefdialog * pd)
{
	listpref_delete_strarr(pd, &pd->od, 7);
}

static void 
duplicate_outputbox_lcb(GtkWidget *wid, Tprefdialog *pd)
{
	listpref_duplicate(pd, &pd->od, 7, set_outputbox_strarr_in_list);
}

static void
reload_extoutputbox(Tprefdialog * pd, GList *list)
{
	listpref_reload(pd, &pd->od, list, 7, set_outputbox_strarr_in_list);
}
...
/*	label = gtk_label_new(NULL);
	...
	hbox = gtk_hbox_new(FALSE, 0);
	gtk_box_pack_start(GTK_BOX(vbox1), hbox, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Add entry"), GTK_STOCK_ADD, G_CALLBACK(add_new_outputbox_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Delete entry"), GTK_STOCK_DELETE, G_CALLBACK(delete_outputbox_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Duplicate entry"), NULL, G_CALLBACK(duplicate_outputbox_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
...
/************ bookmarkcommands code ****************/

static void
set_bookmarkcommands_strarr_in_list(GtkTreeIter * iter, gchar ** strarr, Tprefdialog * pd)
{
	gint arrcount;
	arrcount = g_strv_length(strarr);
	if (arrcount == 8) {
		gtk_list_store_set(GTK_LIST_STORE(pd->bc.lstore), iter,
				0, (strarr[0][0]=='1' || strarr[0][0]=='3'),
				1, strarr[1], 2, strarr[2], 3, strarr[3], 4, strarr[4],
				5, strarr[5], 6, strarr[6],7, strarr[7], 8, strarr, -1);
	}
}

static void
bookmarkcommands_0_edited_lcb(GtkCellRendererToggle * cellrenderertoggle, gchar * path, Tprefdialog * pd)
{
	pref_apply_change(pd->bc.lstore, 8, prefapply_bit1toggle, path, gtk_cell_renderer_toggle_get_active(cellrenderertoggle) ? "0" : "1", 0);
}

static void
bookmarkcommands_1_edited_lcb(GtkCellRendererText * cellrenderertext, gchar * path, gchar * newtext,
					   Tprefdialog * pd)
{
	pref_apply_change(pd->bc.lstore, 8, prefapply_text_w_userdefined, path, newtext, 1);
}

static void
bookmarkcommands_2_edited_lcb(GtkCellRendererText * cellrenderertext, gchar * path, gchar * newtext,
					   Tprefdialog * pd)
{
	pref_apply_change(pd->bc.lstore, 8, prefapply_text_w_userdefined, path, newtext, 2);
}

static void
bookmarkcommands_3_edited_lcb(GtkCellRendererText * cellrenderertext, gchar * path, gchar * newtext,
					   Tprefdialog * pd)
{
	pref_apply_change(pd->bc.lstore, 8, prefapply_text_w_userdefined, path, newtext, 3);
}

static void
bookmarkcommands_4_edited_lcb(GtkCellRendererText * cellrenderertext, gchar * path, gchar * newtext,
					   Tprefdialog * pd)
{
	pref_apply_change(pd->bc.lstore, 8, prefapply_text_w_userdefined, path, newtext, 4);
}

static void
bookmarkcommands_5_edited_lcb(GtkCellRendererText * cellrenderertext, gchar * path, gchar * newtext,
					   Tprefdialog * pd)
{
	pref_apply_change(pd->bc.lstore, 8, prefapply_text_w_userdefined, path, newtext, 5);
}

static void
bookmarkcommands_6_edited_lcb(GtkCellRendererText * cellrenderertext, gchar * path, gchar * newtext,
					   Tprefdialog * pd)
{
	pref_apply_change(pd->bc.lstore, 8, prefapply_text_w_userdefined, path, newtext, 6);
}

static void
bookmarkcommands_7_edited_lcb(GtkCellRendererText * cellrenderertext, gchar * path, gchar * newtext,
					   Tprefdialog * pd)
{
	pref_apply_change(pd->bc.lstore, 8, prefapply_text_w_userdefined, path, newtext, 7);
}

static void
add_new_bookmarkcommands_lcb(GtkWidget * wid, Tprefdialog * pd)
{
	listpref_add_new(pd, &pd->bc, 8, set_bookmarkcommands_strarr_in_list);
}

static void
delete_bookmarkcommands_lcb(GtkWidget * wid, Tprefdialog * pd)
{
	listpref_delete_strarr(pd, &pd->bc, 8);
}

static void 
duplicate_bookmarkcommands_lcb(GtkWidget *wid, Tprefdialog *pd)
{
	listpref_duplicate(pd, &pd->bc, 8, set_bookmarkcommands_strarr_in_list);
}

static void
reload_extbookmarkcoms(Tprefdialog * pd, GList *list)
{
	listpref_reload(pd, &pd->bc, list, 8, set_bookmarkcommands_strarr_in_list);
}

static void
updatedefaults_extbookmarkcoms_lcb(GtkWidget * wid, Tprefdialog * pd)
{
	pd->lists[extbookmarkcoms] = update_bookmarkcommands(pd->lists[extbookmarkcoms], FALSE);
	reload_extbookmarkcoms(pd, pd->lists[extbookmarkcoms]);
}

static void
reset_extbookmarkcoms_lcb(GtkWidget * wid, Tprefdialog * pd)
{
	pd->lists[extbookmarkcoms] = update_bookmarkcommands(pd->lists[extbookmarkcoms], TRUE);
	reload_extbookmarkcoms(pd, pd->lists[extbookmarkcoms]);
}

static gboolean
bookmarkcommands_query_tooltip_lcb(GtkWidget  *widget, gint        x,  gint        y,  gboolean    keyboard_mode,  GtkTooltip *tooltip,  gpointer    user_data)
{
	gtk_tooltip_set_markup(tooltip, _
						 ("<small><b>Input options</b>\n"
							"start with a | to send the input to the standard input\n"
...
static void
create_bookmarkcommands_gui(Tprefdialog * pd, GtkWidget * vbox1)
{
	GtkWidget *hbox, *but, *scrolwin;
	pd->lists[extbookmarkcoms] = duplicate_arraylist(main_v->props.external_bookmarkcommands);
	pd->bc.lstore =
		gtk_list_store_new(9, G_TYPE_BOOLEAN, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
						   G_TYPE_STRING, G_TYPE_STRING, G_TYPE_POINTER);
	pd->bc.lview = gtk_tree_view_new_with_model(GTK_TREE_MODEL(pd->bc.lstore));
	g_object_unref(G_OBJECT(pd->bc.lstore));
	pref_create_column(GTK_TREE_VIEW(pd->bc.lview), 2, G_CALLBACK(bookmarkcommands_0_edited_lcb), pd, _("Enabled"), 0,
					   FALSE);
	pref_create_column(GTK_TREE_VIEW(pd->bc.lview), 1, G_CALLBACK(bookmarkcommands_1_edited_lcb), pd, _("Name"), 1,
					   FALSE);
	pref_create_column(GTK_TREE_VIEW(pd->bc.lview), 1, G_CALLBACK(bookmarkcommands_2_edited_lcb), pd, _("Pattern"),
					   2, FALSE);
	pref_create_column(GTK_TREE_VIEW(pd->bc.lview), 1, G_CALLBACK(bookmarkcommands_3_edited_lcb), pd, _("File #"), 3,
					   FALSE);
	pref_create_column(GTK_TREE_VIEW(pd->bc.lview), 1, G_CALLBACK(bookmarkcommands_4_edited_lcb), pd, _("Line #"), 4,
					   FALSE);
	pref_create_column(GTK_TREE_VIEW(pd->bc.lview), 1, G_CALLBACK(bookmarkcommands_5_edited_lcb), pd, _("Output #"),
					   5, FALSE);
	pref_create_column(GTK_TREE_VIEW(pd->bc.lview), 1, G_CALLBACK(bookmarkcommands_6_edited_lcb), pd, _("Command"),
					   6, FALSE);
	pref_create_column(GTK_TREE_VIEW(pd->bc.lview), 1, G_CALLBACK(bookmarkcommands_7_edited_lcb), pd, _("Color"),
					   7, FALSE);	
	g_signal_connect(G_OBJECT(pd->bc.lview), "query-tooltip", G_CALLBACK(bookmarkcommands_query_tooltip_lcb), pd);
	g_object_set(G_OBJECT(pd->bc.lview), "has-tooltip", TRUE, NULL);
	scrolwin = gtk_scrolled_window_new(NULL, NULL);
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolwin), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(scrolwin), GTK_SHADOW_IN);
	gtk_container_add(GTK_CONTAINER(scrolwin), pd->bc.lview);
	gtk_box_pack_start(GTK_BOX(vbox1), scrolwin, TRUE, TRUE, 2);
	reload_extbookmarkcoms(pd, pd->lists[extbookmarkcoms]);
	gtk_tree_view_set_reorderable(GTK_TREE_VIEW(pd->bc.lview), TRUE);
	pd->bc.thelist = &pd->lists[extbookmarkcoms];
	pd->bc.insertloc = -1;
	g_signal_connect(G_OBJECT(pd->bc.lstore), "row-inserted", G_CALLBACK(listpref_row_inserted), &pd->bc);
	g_signal_connect(G_OBJECT(pd->bc.lstore), "row-deleted", G_CALLBACK(listpref_row_deleted), &pd->bc);

	hbox = gtk_hbox_new(FALSE, 0);
	gtk_box_pack_start(GTK_BOX(vbox1), hbox, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Add entry"), GTK_STOCK_ADD, G_CALLBACK(add_new_bookmarkcommands_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Delete entry"), GTK_STOCK_DELETE, G_CALLBACK(delete_bookmarkcommands_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Duplicate entry"), NULL, G_CALLBACK(duplicate_bookmarkcommands_lcb), pd, FALSE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Update defaults"), GTK_STOCK_REFRESH, G_CALLBACK(updatedefaults_extbookmarkcoms_lcb), pd, TRUE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
	but = dialog_button_new_with_image(_("Reset all"), GTK_STOCK_CONVERT, G_CALLBACK(reset_extbookmarkcoms_lcb), pd, TRUE, FALSE);
	gtk_box_pack_start(GTK_BOX(hbox), but, FALSE, FALSE, 2);
}
...
{
	listpref_delete_strarr(pd, &pd->tg, 2);
}
...
preferences_destroy_lcb(GtkWidget * widget, Tprefdialog * pd)
{
	...
	free_arraylist(pd->lists[extbookmarkcoms]);
	pd->lists[extbookmarkcoms] = NULL;
#ifdef OLDTEMPLATES
...
preferences_apply(Tprefdialog * pd)
{
	...
	free_arraylist(main_v->props.external_bookmarkcommands);
	main_v->props.external_bookmarkcommands = duplicate_arraylist(pd->lists[extbookmarkcoms]);
#ifdef OLDTEMPLATES
...
preftree_cursor_changed_cb(GtkTreeView * treeview, gpointer user_data)
{
	...
	if (path) {
		gpointer data = NULL;
		void (*func) (Tprefdialog *, gpointer);
...
preferences_dialog_new(Tbfwin *bfwin)
{
	...
	gtk_tree_store_append(pd->nstore, &auxit, NULL);
	gtk_tree_store_set(pd->nstore, &auxit, NAMECOL, _("Bookmark commands"), WIDGETCOL, frame, -1);
	pd->widgetfreelist = g_slist_prepend(pd->widgetfreelist, frame);
	dialog_label_new(_("<b>Bookmark commands</b>"), 0, 0, vbox1, 4);
	create_bookmarkcommands_gui(pd, vbox1);
	/*
	 *  Plugins
	 */
	frame = gtk_frame_new(NULL);
